...
def elementZtoName(Z:int):          # Returns Element name 
...
def main():
    ...
    if USE_COLOURS_IN_TERMINAL:
        while loop == True:
            ...
            cont = False
            cont2 = False
            element_symbol_given = False
            while cont == False:
                request = input(f'{bcol.OKCYAN}Enter energy of suspected anomaly (eV), or an Element Symbol (e.g. Fe): {bcol.ENDC}')
                try:
                    ...
                    try:
                        ...
                        test1 = request_lines[0] # errors if nothing was added to request_lines (i.e. input was not valid element symbol)
                        element_symbol_given = True
                        cont = True
                    except:
                        print(f'{bcol.WARNING}Energy must be either a Float or Integer eV value, or a recognised Element Symbol.{bcol.ENDC}')
                        cont = False
                        element_symbol_given = False
            ...
            print('')
            if element_symbol_given:
                print(f'Displaying nearby spectral phenomena for each of the {len(request_lines)} common spectral lines of {elementSymboltoName(request)}:')
                for lin in request_lines:
                    print(f'{bcol.UNDERLINE}{qty} nearest possibilities to {bcol.OKBLUE}{lin} eV{bcol.ENDC}:')
                    print(getClosestByEnergy(lin, qty))
            else:
                print(f'{bcol.UNDERLINE}{qty} nearest possibilities to {bcol.OKBLUE}{request} eV{bcol.ENDC}:')
        ...
        while loop == True:
            ...
            element_symbol_given = False
            while cont == False:
                request = input('Enter energy of suspected anomaly (eV), or an Element Symbol (e.g. Fe): : ')
                try:
                    request = float(request)
                    cont = True
                except:
                    try:
                        ...
                        print('Energy must be either a Float or Integer eV value, or a recognised Element Symbol.')
                        cont = False
            ...
            print('')
            if element_symbol_given:
                print(f'Displaying nearby spectral phenomena for each of the {len(request_lines)} common spectral line(s) of {request}:')
                for lin in request_lines:
                    print(f'{qty} nearest possibilities to {lin} eV:')
                    print(getClosestByEnergy(lin, qty))
            else:
                print(f'{qty} nearest possibilities to {request} eV:')
                print(getClosestByEnergy(request, qty))
            print('')
            input('Press Enter to Continue...')
