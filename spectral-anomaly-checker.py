...
def main():
	...
	print(f'{bcol.OKGREEN}Energy data loaded from file.{bcol.ENDC}')
	...
	while loop:
		...
		while True:
			request_str = input(f'{bcol.OKCYAN}Energy of anomaly in eV or element symbol (e.g. `11520` or `Fe`): {bcol.ENDC}')
			...
			try:
				...
				try:
					...
					for line in all_standard_element_lines:
						if line[0] == request_str:
							request_lines.append(line[2]*1000)
					request_lines[0] # errors if nothing was added to request_lines (i.e. input was not valid element symbol)
					element_symbol_given = True
					break
				except Exception as e:
					print(f'{bcol.WARNING}Energy must be either a Float or Integer eV value, or a recognised Element Symbol. ({e}) {bcol.ENDC}')
					element_symbol_given = False

		qty_str:str = input(f'{bcol.OKCYAN}Enter the number of nearby possibilities to display (default=5): {bcol.ENDC}')
		...
		if element_symbol_given:
			...
			print(f'Displaying nearby spectral phenomena for each of the {len(request_lines)} common spectral lines of {elementSymboltoName(request_str)}:')
			...
			# eV value provided
			print(f'{bcol.UNDERLINE}{qty} nearest possibilities to {bcol.OKBLUE}{request_ev} eV{bcol.ENDC}:')
			print(getClosestByEnergy(request_ev, qty))
